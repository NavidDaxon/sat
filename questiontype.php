...
/**
 * The questiontype class for the SAT Exam question type.
 *
 * @package    qtype
 * @subpackage satexam
...
/**
 * The SAT Exam question type.
 *
 * @copyright  1999 onwards Martin Dougiamas  {@link http://moodle.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_satexam extends question_type {
    public function get_question_options($question) {
        global $DB, $OUTPUT;

        $question->options = $DB->get_record('qtype_satexam_options', ['questionid' => $question->id]);
    ...
    protected function create_default_options($question) {
        ...
        $config = get_config('qtype_satexam');
    ...
    public function save_question_options($question) {
        ...
        // if ($answercount < 2) { // Check there are at lest 2 answers for SAT Exam.
        //     $result->error = get_string('notenoughanswers', 'qtype_satexam', '2');
        //     return $result;
        // }
        ...
        $options = $DB->get_record('qtype_satexam_options', array('questionid' => $question->id));
        if (!$options) {
            $options = new stdClass();
            $options->questionid = $question->id;
            ...
            $options->showstandardinstruction = 0;
            $options->id = $DB->insert_record('qtype_satexam_options', $options);
        }
        ...
        $DB->update_record('qtype_satexam_options', $options);
        ...
        if ($options->single) {
            if ($maxfraction != 1) {
                $result->noticeyesno = get_string('fractionsnomax', 'qtype_satexam',
                        $maxfraction * 100);
                return $result;
            }
        } else {
            ...
            if ($totalfraction != 1) {
                $result->noticeyesno = get_string('fractionsaddwrong', 'qtype_satexam',
                        $totalfraction * 100);
                return $result;
            }
        }
    ...
    protected function make_question_instance($questiondata) {
        ...
        if ($questiondata->options->single) {
            $class = 'qtype_satexam_single_question';
        } else {
            $class = 'qtype_satexam_multi_question';
        }
        return new $class();
    }
    ...
    protected function initialise_question_instance(question_definition $question, $questiondata) {
        ...
        if (!empty($questiondata->options->layout)) {
            ...
            $question->layout = qtype_satexam_single_question::LAYOUT_VERTICAL;
        }
    ...
    public function delete_question($questionid, $contextid) {
        global $DB;
        $DB->delete_records('qtype_satexam_options', array('questionid' => $questionid));
     ...
     *      should be a lang string answernumberingxxx in teh qtype_satexam
    ...
    public static function get_numbering_styles() {
        ...
        foreach (array('abc', 'ABCD', '123', 'iii', 'IIII', 'none') as $numberingoption) {
            $styles[$numberingoption] =
                    get_string('answernumbering' . $numberingoption, 'qtype_satexam');
        }
        return $styles;
    }
