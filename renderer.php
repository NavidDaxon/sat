...
/**
 * SAT Exam question renderer classes.
 *
 * @package    qtype
 * @subpackage satexam
...
/**
 * Base class for generating the bits of output common to SAT Exam
 * single and multiple questions.
 *
 * @copyright  2009 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class qtype_satexam_renderer_base extends qtype_with_combined_feedback_renderer {
    ...
    public function formulation_and_controls(question_attempt $qa,
        ...
        $this->page->requires->js_call_amd('qtype_satexam/answers', 'init',
     ...
     * options returned by {@link qtype_satexam:;get_numbering_styles()}.
    ...
    protected function correct_choices(array $right) {
        ...
        if (count($right) == 1) {
                return get_string('correctansweris', 'qtype_satexam',
                        implode(', ', $right));
        } else if (count($right) > 1) {
                return get_string('correctanswersare', 'qtype_satexam',
                        implode(', ', $right));
        } else {
                return "";
        }
...
/**
 * Subclass for generating the bits of output specific to SAT Exam
 * single questions.
 *
 * @copyright  2009 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_satexam_single_renderer extends qtype_satexam_renderer_base {
    ...
    protected function prompt() {
        return get_string('selectone', 'qtype_satexam');
    }
    ...
    public function after_choices(question_attempt $qa, question_display_options $options) {
        ...
        $clearchoicewrapperattrs = [
            ...
            'class' => 'qtype_satexam_clearchoice',
        ];
        ...
        $clearchoice = html_writer::link('#', get_string('clearchoice', 'qtype_satexam'),
        ...
        $this->page->requires->js_call_amd('qtype_satexam/clearchoice', 'init',
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_satexam_multi_renderer extends qtype_satexam_renderer_base {
    ...
    protected function prompt() {
        return get_string('selectmulti', 'qtype_satexam');
    }
    ...
    protected function num_parts_correct(question_attempt $qa) {
        if ($qa->get_question()->get_num_selected_choices($qa->get_last_qt_data()) >
            ...
            return get_string('toomanyselected', 'qtype_satexam');
        }
